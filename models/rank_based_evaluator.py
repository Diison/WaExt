...
from pykeen.evaluation.evaluator import Evaluator, MetricResults, prepare_filter_triples
...
class WeightAwareRankBasedEvaluator(Evaluator):
    """A weight-aware rank-based evaluator for KGE models."""
    ...
    # def weighting_func(self, weight, c):
    #     return c/(c+weight)
    ...
    def process_scores_(
        ...
        self.num_entities = scores.shape[1]
        for rank_type, v in batch_ranks.items():
            weight = torch.FloatTensor([ self.weighting_func(-self.quads.get(tuple(hrt_batch[i, :].tolist()), 0)) for i in range(hrt_batch.size()[0])]).to(device=hrt_batch.device)
            real_w = [ self.quads.get(tuple(hrt_batch[i, :].tolist()), 0) for i in range(hrt_batch.size()[0])]
            if rank_type=="realistic":
                for i in range(v.size()[0]):
                    self.outter.writerow([int(v[i].item()), real_w[i], hrt_batch[i, 0].item(), hrt_batch[i, 1].item(), hrt_batch[i, 2].item()])
            v=v.mul(weight)
            self.ranks[target, rank_type].append(v.detach().cpu().numpy())
        self.num_candidates[target].append(batch_ranks.number_of_options.detach().cpu().numpy())
