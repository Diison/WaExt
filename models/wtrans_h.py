...
class WTransH(EntityRelationEmbeddingModel):
    ...
    #: The custom regularizer used by [wang2014]_ for WTransH
    ...
    def score_hrt(self, hrt_batch: torch.LongTensor, **kwargs) -> torch.FloatTensor:  # noqa: D102
        ...
        dist=-linalg.vector_norm(ph + d_r - pt, ord=2, dim=-1, keepdim=True)
        ...
        # w = torch.FloatTensor([ self.weighting_func(self.quads.get(tuple(hrt_batch[i, :].tolist()), self.n_weight), 2) for i in range(hrt_batch.size()[0])]).to(device=self.mydevice)
