...
"""Get triples from the CN15K dataset."""
...
CN15K_TRAIN_PATH = HERE.joinpath("train.tsv")
CN15K_TEST_PATH = HERE.joinpath("test.tsv")
CN15K_VALIDATE_PATH = HERE.joinpath("val.tsv")

@parse_docdata
class CN15K(LazyDataset):
    ...
    def __init__(self, create_inverse_triples: bool = False, th=0, pct=1, **kwargs):
        ...
        self.train_quad = self.load_quadruples(CN15K_TRAIN_PATH)
        self.test_quad = self.load_quadruples(CN15K_TEST_PATH)
        self.val_quad = self.load_quadruples(CN15K_VALIDATE_PATH)
    ...
    def _load(self) -> None:
        ...
        # self._training = TriplesFactory(mapped_triples = torch.LongTensor(tuple(self.train_quad.keys())), entity_to_id = self.index_ents, relation_to_id = self.index_rels, create_inverse_triples=False)
    ...
    def __repr__(self) -> str:  # noqa: D105
        return (
            f'{self.__class__.__name__}(training_path="{CN15K_TRAIN_PATH}", testing_path="{CN15K_TEST_PATH}",'
            f' validation_path="{CN15K_VALIDATE_PATH}")'
        )

def load_cn15k(th=0, pct=100, create_inverse_triples=False):
    return CN15K(th=th, pct=pct, create_inverse_triples=create_inverse_triples)
